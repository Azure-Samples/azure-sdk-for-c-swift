  ...
  else
  {
    if (azure_iot->mqtt_client_handle != NULL)
    {
      if (azure_iot->config->mqtt_client_interface.mqtt_client_deinit(
        azure_iot->config->user_context, azure_iot->mqtt_client_handle) != 0)
      {
  ...
  switch (azure_iot->state)
  {
    ...
    case azure_iot_state_started:
      ...
      if (result != 0 || 
          azure_iot->config->mqtt_client_interface.mqtt_client_init(
            azure_iot->config->user_context, &mqtt_client_config, &azure_iot->mqtt_client_handle) != 0)
      {
        azure_iot->state = azure_iot_state_error;
    ...
    case azure_iot_state_subscribed_to_dps:
      ...
      packet_id = azure_iot->config->mqtt_client_interface.mqtt_client_publish(
        azure_iot->config->user_context, azure_iot->mqtt_client_handle, &mqtt_message);
      ...
      azure_iot->dps_last_query_time = get_current_unix_time();
...
int azure_iot_mqtt_client_disconnected(azure_iot_t* azure_iot)
{
  ...
  if (azure_iot->state == azure_iot_state_refreshing_sas)
  {
    ...
    result = RESULT_OK;
           ...
           azure_iot->config->use_device_provisioning)
  {
    // This is the disconnection from DPS, before connecting to IoT Hub.
    result = RESULT_OK;
  }
  else
  {
...
static int get_mqtt_client_config_for_dps(azure_iot_t* azure_iot, mqtt_client_config_t* mqtt_client_config)
{
  ...
  password_span = az_span_slice(password_span, 0, password_length);
  ...
  client_id_span = az_span_slice(client_id_span, 0, client_id_length);
  ...
  username_span = az_span_slice(username_span, 0, username_length);
