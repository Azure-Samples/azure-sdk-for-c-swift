...
typedef struct mqtt_client_config_t_struct
{
   ...
   * @remark   This string is null-terminated.
   */
  az_span address;
 ...
 *
 * @param[in]     user_context          A pointer to the user context provided in the Azure IoT configuration.
 ...
 */
typedef int (*mqtt_client_init_function_t)(const void* user_context, mqtt_client_config_t* mqtt_client_config, mqtt_client_handle_t* mqtt_client_handle);
 ...
 */
typedef int (*mqtt_client_deinit_function_t)(const void* user_context, mqtt_client_handle_t mqtt_client_handle);
 ...
 */
typedef int (*mqtt_client_publish_function_t)(const void* user_context, mqtt_client_handle_t mqtt_client_handle, mqtt_message_t* mqtt_message);
 ...
 */
typedef int (*mqtt_client_subscribe_function_t)(const void* user_context, mqtt_client_handle_t mqtt_client_handle, az_span topic, mqtt_qos_t qos);
...
typedef struct azure_iot_config_t_struct
{
   ...
   * @remark    This string is not null-terminated.
   *            This string is not exposed back to the user device or service applications,
   ...
   * @remark    This string is not null-terminated.
   *            If performing device-provisioning this MUST be set to AZ_SPAN_EMPTY;
   ...
   * @remark    This string is not null-terminated.
   *            This key will be used to generate the MQTT client password, if using SAS-token
   ...
   * @remark    This string is not null-terminated.
   *            This information is only needed when performing device-provisioning (which is used,
  ...
  /*
   * @brief     A pointer to any structure the caller would like to use as context for Azure IoT callbacks.
   */
